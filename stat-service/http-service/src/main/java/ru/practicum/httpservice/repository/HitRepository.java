...
public interface HitRepository extends JpaRepository<HitEntity, Long> {
    @Query("SELECT new ru.practicum.dto.ViewStats(s.app.name, s.uri, COUNT(DISTINCT s.ip)) " +
            "FROM HitEntity  AS s " +
    ...
    @Query("SELECT new ru.practicum.dto.ViewStats(s.app.name, s.uri, COUNT(s.id)) " +
            "FROM HitEntity  AS s " +
            "WHERE s.timeStamp BETWEEN ?1 AND ?2 " +
            "GROUP BY s.app, s.uri " +
